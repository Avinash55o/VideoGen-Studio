...
directly via URL; older stdio-only clients use the ``videogen-mcp`` shim
...
def build_mcp_server() -> FastMCP:
    """Create the FastMCP instance with VideoGen tools registered."""
    mcp = FastMCP(
        name="videogen",
        instructions=(
            "VideoGen Studio is a local video and voice AI layer. Use `videogen.speak` to "
            "play text in a voice profile, `videogen.transcribe` for "
...
def compose_lifespan(*lifespans):
    ...
    Used by ``create_app`` to run the existing VideoGen startup/shutdown
