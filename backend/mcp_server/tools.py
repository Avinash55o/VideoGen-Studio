"""VideoGen Studio MCP tool implementations.

Thin wrappers over existing services/routes. Tools are registered with dotted
names (``videogen.speak`` etc.) so they look natural in agent logs —
...
def register_tools(mcp: FastMCP) -> None:
    """Attach all VideoGen tools to the given FastMCP instance."""

    @mcp.tool(
        name="videogen.speak",
        description=(
            "Speak text in a voice profile. Returns a generation id "
            "the caller can poll at /generate/{id}/status. Audio plays on the "
            "user's speakers and is saved to history."
        ),
    )
    async def videogen_speak(
        text: str,
    ...
    @mcp.tool(
        name="videogen.transcribe",
        description=(
            "Transcribe an audio clip to text using the local Whisper engine. "
        ),
    )
    async def videogen_transcribe(
        ...
        # to loopback callers so a VideoGen server bound on 0.0.0.0 doesn't double
    ...
    @mcp.tool(
        name="videogen.list_captures",
    )
    async def videogen_list_captures(
    ...
    @mcp.tool(
        name="videogen.list_profiles",
        description=(
            "List available voice profiles (cloned + preset). "
            "Use the returned `name` with videogen.speak(profile=...)."
        ),
    )
    async def videogen_list_profiles() -> dict[str, Any]:
        db = next(get_db())
        try:
...
async def _transcribe_file(
    ...
    if (
        ...
        raise ValueError(
            ...
            "VideoGen Studio → Settings → Models to download it first."
        )
