...
MCP clients identify themselves via an ``X-Videogen-Client-Id`` HTTP header
(direct-HTTP clients set it in their MCP config; the stdio shim forwards it
from the ``VIDEOGEN_CLIENT_ID`` env var). Middleware copies the value into a
...
# host-filesystem access to loopback callers (see videogen.transcribe).
...
# Endpoints that consume X-Videogen-Client-Id for its MCP-semantic
...
#
# - /mcp — FastMCP tool calls (videogen.speak, videogen.transcribe, …)
...
#   false stamps.
# - /speak — REST mirror of videogen.speak for non-MCP agents (shell
...
class ClientIdMiddleware(BaseHTTPMiddleware):
    """Copy X-Videogen-Client-Id into a ContextVar and stamp last_seen_at
