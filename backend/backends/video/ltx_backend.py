"""LTX-Video generation backend using diffusers."""
...
class LTXVideoBackend:
    ...
    def _load_model_sync(self) -> None:
        ...
        logger.info("Loading LTX-Video from %s", repo)
        is_cached = is_model_cached(repo)

        with model_load_progress("ltx-video", is_cached):
            self._pipe = LTXVideoPipeline.from_pretrained(
                repo,
    ...
    def _apply_memory_optimizations(self) -> None:
        ...
        if vram_gb is not None and vram_gb < 16:
            logger.info("LTX-Video enabling model CPU offload")
    ...
    async def generate_from_image(
        ...
        # LTXVideoPipeline natively accepts 'image' parameter for Image-to-Video
        output = self._pipe(
            prompt=prompt,
            image=image,
            num_frames=num_frames,
