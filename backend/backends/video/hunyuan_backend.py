"""HunyuanVideo generation backend using diffusers."""
...
class HunyuanVideoBackend:
    """HunyuanVideo generation backend."""
    ...
    def _load_model_sync(self) -> None:
        ...
        logger.info("Loading HunyuanVideo from %s", repo)
        is_cached = is_model_cached(repo)

        with model_load_progress("hunyuan-video", is_cached):
            self._pipe = HunyuanVideoPipeline.from_pretrained(
                repo,
                torch_dtype=self._dtype,
            )
            self._apply_memory_optimizations()
            logger.info("HunyuanVideo model loaded")

    def _apply_memory_optimizations(self) -> None:
        vram_gb = get_vram_gb()
        if vram_gb is not None and vram_gb < 24:
            logger.info("HunyuanVideo enabling model CPU offload")
    ...
    async def generate_from_image(
        ...
        raise NotImplementedError("Image-to-Video generation is not supported for HunyuanVideo")
