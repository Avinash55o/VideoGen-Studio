...
VIDEO_ENGINES = {
    "cogvideo": "CogVideoX 5B I2V",
    "cogvideo-2b": "CogVideoX 2B T2V",
    "wan-t2v": "Wan2.1 T2V 1.3B",
    "wan-i2v": "Wan2.1 I2V 14B",
    "ltx": "LTX-Video (T2V/I2V)",
    "hunyuan": "HunyuanVideo (T2V)",
    "mochi": "Mochi-1 Preview (T2V)",
    "svd": "Stable Video Diffusion XT",
}


def _get_video_model_configs() -> list[ModelConfig]:
    """Return video generation model configs."""
    return [
        ModelConfig(
        ...
        ModelConfig(
            ...
            display_name="LTX-Video (T2V/I2V)",
            engine="ltx",
...
def get_all_model_configs() -> list[ModelConfig]:
    """Return the full list of model configs (video + TTS + STT + LLM)."""
    return (
        _get_video_model_configs()
...
def get_video_model_configs() -> list[ModelConfig]:
    """Return only video generation model configs."""
    return _get_video_model_configs()
...
def unload_model_by_config(config: ModelConfig) -> bool:
    ...
    if config.engine in VIDEO_ENGINES:
        from .video import get_video_backend
        backend = get_video_backend(config.engine)
