"""
Configuration module for VideoGen Studio backend.
...
# Set VIDEOGEN_MODELS_DIR to an absolute path before starting the server.
# This sets HF_HUB_CACHE so all huggingface_hub downloads go to that path.
_custom_models_dir = os.environ.get("VIDEOGEN_MODELS_DIR")
...
def get_db_path() -> Path:
    ...
    return _data_dir / "videogen.db"


def get_profiles_dir() -> Path:
    """Get voice profiles directory path."""
    path = _data_dir / "voice-profiles"
...
def get_projects_dir() -> Path:
    ...
    path = _data_dir / "projects"
    path.mkdir(parents=True, exist_ok=True)
    return path


def get_renders_dir() -> Path:
    """Get renders directory path."""
    path = _data_dir / "renders"
    path.mkdir(parents=True, exist_ok=True)
    return path
