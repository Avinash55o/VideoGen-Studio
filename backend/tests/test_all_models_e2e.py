...
def find_binary() -> Optional[Path]:
    ...
    candidates = [
        DIST_DIR / "videogen-server-cuda" / f"videogen-server-cuda{exe}",
        DIST_DIR / f"videogen-server{exe}",
    ]
    for c in candidates:
...
def write_reports(
    ...
    lines = [
        f"# VideoGen Studio E2E — {plat} — {started_at.strftime('%Y-%m-%d %H:%M UTC')}",
        "",
        f"Binary: `{binary}`  ",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="VideoGen Studio E2E model generation test")
    p.add_argument("--binary", type=Path, help="Path to videogen-server binary (overrides auto-detect)")
...
def main() -> int:
    ...
    # Tempdir + log path
    data_dir = Path(tempfile.mkdtemp(prefix="videogen-e2e-"))
