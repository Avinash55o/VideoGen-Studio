...
|----------|------|------------|
| macOS    | `backend/dist/videogen-server-cuda/videogen-server-cuda` | onedir (CUDA, rarely on Mac) |
| macOS    | `backend/dist/videogen-server`                          | onefile (CPU) |
| Windows  | `backend\dist\videogen-server-cuda\videogen-server-cuda.exe` | onedir (CUDA) |
| Windows  | `backend\dist\videogen-server.exe`                      | onefile (CPU) |
...
- **Data dir**: `tempfile.mkdtemp(prefix="videogen-e2e-")`. Deleted after the run unless `--keep-data-dir`. Profiles and generated WAVs land here.
...
```json
{
  ...
  "binary": "/abs/path/videogen-server",
  "binary_size_mb": 612,
...
```
# VideoGen Studio E2E — darwin-arm64 — 2026-04-16 12:34
