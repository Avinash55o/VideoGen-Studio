"""POST /speak — REST wrapper around videogen.speak for non-MCP callers.
...
bindings (via X-Videogen-Client-Id) work identically.
"""
...
async def speak(
    ...
    """Speak text in a voice profile. Mirrors videogen.speak (MCP).
    ...
    """
    client_id = request.headers.get("X-Videogen-Client-Id")
    ...
    if profile is None:
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                "or configure a default in VideoGen Studio → Settings → MCP."
            ),
        )
