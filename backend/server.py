"""
Entry point for PyInstaller-bundled VideoGen Studio server.
...
if "--version" in sys.argv:
    ...
    print(f"videogen-server {__version__}")
    sys.exit(0)
...
logger.info("=" * 60)
logger.info("videogen-server starting up...")
...
if __name__ == "__main__":
    try:
        parser = argparse.ArgumentParser(description="VideoGen Studio backend server")
        parser.add_argument(
            "--host",
            type=str,
            default="127.0.0.1",
        ...
        # videogen-server-cuda → sets VIDEOGEN_BACKEND_VARIANT=cuda
        import os
        binary_name = os.path.basename(sys.executable).lower()
        if "cuda" in binary_name:
            os.environ["VIDEOGEN_BACKEND_VARIANT"] = "cuda"
            logger.info("Backend variant: CUDA")
        else:
            os.environ["VIDEOGEN_BACKEND_VARIANT"] = "cpu"
