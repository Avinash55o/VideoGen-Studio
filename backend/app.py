"""FastAPI application factory, middleware, and lifecycle events for VideoGen Studio."""
...
def create_app() -> FastAPI:
    ...
    @asynccontextmanager
    async def videogen_lifespan(app: FastAPI):
    ...
    # compose_lifespan enters factories in order (videogen startup →
    ...
    lifespan = compose_lifespan(videogen_lifespan, mcp_app.router.lifespan_context)

    application = FastAPI(
        title="VideoGen Studio API",
        description="Local-first AI video generation studio",
        version=__version__,
        lifespan=lifespan,
    )
...
def _configure_cors(application: FastAPI) -> None:
    ...
    env_origins = os.environ.get("VIDEOGEN_CORS_ORIGINS", "")
...
async def _run_startup(application: FastAPI) -> None:
    ...
    logger.info("VideoGen Studio v%s starting up", __version__)
    logger.info(
        "Python %s on %s %s (%s)",
        sys.version.split()[0],
        platform.system(),
    ...
    try:
        ...
        if result.rowcount > 0:
            logger.info("Marked %d stale render(s) as failed", result.rowcount)

        from .database import Project as DBProject

        project_count = db.query(DBProject).filter(DBProject.deleted_at.is_(None)).count()
        logger.info("Projects: %d", project_count)
...
async def _run_shutdown() -> None:
    ...
    logger.info("VideoGen Studio server shutting down...")
    try:
