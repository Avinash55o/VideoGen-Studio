...
def build_server(cuda=False):
    ...
    Args:
        cuda: If True, build with CUDA support and name the binary
              videogen-server-cuda instead of videogen-server.
    """
    backend_dir = Path(__file__).parent

    binary_name = "videogen-server-cuda" if cuda else "videogen-server"
    ...
    args.extend(
        [
            ...
            "--hidden-import",
            "loguru",
            # MCP server — Streamable-HTTP endpoint and the 4 videogen.* tools.
...
def build_shim():
    """Build the videogen-mcp stdio shim as a tiny standalone binary.
    This shim forwards JSON-RPC to the main videogen-server's /mcp
    endpoint. Keep it small: no torch, no ML deps, just httpx + asyncio.
    ...
    os.chdir(backend_dir)
    PyInstaller.__main__.run(args)
    logger.info("Shim built: %s", backend_dir / "dist" / "videogen-mcp")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build VideoGen binaries")
        help="Build CUDA-enabled binary (videogen-server-cuda)",
        help="Build the videogen-mcp stdio shim binary instead of the server",
    )
