"""videogen-mcp — stdio ↔ Streamable-HTTP MCP proxy.
...
server's response back. The VideoGen server does all the real work.

Environment variables:
  VIDEOGEN_PORT       VideoGen server port (default 17493).
  VIDEOGEN_HOST       Host (default 127.0.0.1).
  VIDEOGEN_CLIENT_ID  Forwarded as X-Videogen-Client-Id on every request.
...
CLIENT_ID_HEADER = "X-Videogen-Client-Id"
...
def _err(msg: str) -> None:
    print(f"videogen-mcp: {msg}", file=sys.stderr, flush=True)


def _base_url() -> tuple[str, str]:
    host = os.environ.get("VIDEOGEN_HOST", "127.0.0.1")
    port = int(os.environ.get("VIDEOGEN_PORT", str(DEFAULT_PORT)))
            ...
            _write_stdout(
                {
                    ...
                    "error": {
                        "code": -32000,
                        "message": (
                            f"VideoGen MCP proxy got HTTP {response.status_code}"
                        ),
                    },
                }
            )
...
async def _run() -> int:
    ...
    client_id = os.environ.get("VIDEOGEN_CLIENT_ID")
    if client_id:
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(300.0)) as client:
        if not await _wait_for_backend(client, health_url):
            _err(
                f"timed out waiting for VideoGen at {health_url} — is the app open?"
            )
            return 2
