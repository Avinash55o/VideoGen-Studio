...
export interface GenerationSettings {
  ...
  output_fps: number;
  video_codec: string;
  max_render_duration_secs: number;
}
...
/* ─── VideoGen Studio (Projects, Clips, Generation) ────────────────────── */
...
export type ClipType = 'video' | 'voiceover' | 'music' | 'subtitle' | 'image' | 'text';
...
export interface TimelineBatchUpdate {
  clips: TimelineClipPosition[];
}
...
export interface VoiceProfileResponseV2 {
  id: string;
  name: string;
  description?: string;
...
export interface AudioLibraryEntryResponse {
  id: string;
  name: string;
  file_path: string;
...
export interface SubtitleTrackUpdate {
  items?: SubtitleItem[];
  style?: Record<string, unknown>;
}

export interface VoiceProfileCreateV2 {
  name: string;
  description?: string;
  voice_type?: 'cloned' | 'preset' | 'designed';
  preset_engine?: string;
