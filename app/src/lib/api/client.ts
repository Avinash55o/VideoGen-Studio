...
class ApiClient {
  ...
  async listClips(projectId: string): Promise<ClipResponse[]> {
    return this.request<ClipResponse[]>(`/api/projects/${projectId}/clips`);
  }

  async getClip(projectId: string, clipId: string): Promise<ClipResponse> {
    return this.request<ClipResponse>(`/api/projects/${projectId}/clips/${clipId}`);
  }

  async createClip(projectId: string, data: ClipCreate): Promise<ClipResponse> {
    return this.request<ClipResponse>(`/api/projects/${projectId}/clips`, {
      method: 'POST',
      body: JSON.stringify(data),
    });
  ...
  async updateClip(projectId: string, clipId: string, data: ClipUpdate): Promise<ClipResponse> {
    return this.request<ClipResponse>(`/api/projects/${projectId}/clips/${clipId}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    });
  ...
  async deleteClip(projectId: string, clipId: string): Promise<void> {
    await this.request<void>(`/api/projects/${projectId}/clips/${clipId}`, {
      method: 'DELETE',
    });
  }

  async batchUpdateTimeline(projectId: string, positions: TimelineClipPosition[]): Promise<void> {
    await this.request<void>(`/api/projects/${projectId}/clips/timeline`, {
      method: 'PUT',
      body: JSON.stringify({ clips: positions }),
    });
  }
  ...
  getGenerationProgressUrl(taskId: string): string {
    return `${this.getBaseUrl()}/api/generate/progress/${taskId}`;
  }
  ...
  async generateVoiceover(data: {
    ...
    language?: string;
    engine?: string;
  }): Promise<{ clip_id: string; task_id: string; status: string }> {
    return this.request('/api/voiceover/generate', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }
  ...
  async generateSubtitles(data: {
    ...
    language?: string;
    stt_model?: string;
  }): Promise<import('./types').SubtitleTrackResponse> {
    return this.request('/api/subtitles/generate', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }

  async updateSubtitleTrack(
    trackId: string,
    data: import('./types').SubtitleTrackUpdate,
  ): Promise<import('./types').SubtitleTrackResponse> {
    return this.request(`/api/subtitles/${trackId}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    });
  }

  // ── Voice Profiles (VideoGen v2) ───────────────────────────────────

  async listVoiceProfilesV2(): Promise<import('./types').VoiceProfileResponseV2[]> {
    return this.request('/api/voice-profiles');
  }

  async getVoiceProfileV2(profileId: string): Promise<import('./types').VoiceProfileResponseV2> {
    return this.request(`/api/voice-profiles/${profileId}`);
  }

  async createVoiceProfileV2(
    data: import('./types').VoiceProfileCreateV2,
  ): Promise<import('./types').VoiceProfileResponseV2> {
    return this.request('/api/voice-profiles', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  ...
  async listAudioLibrary(): Promise<import('./types').AudioLibraryEntryResponse[]> {
    return this.request('/api/audio-library');
  }
  ...
  async startRender(
    projectId: string,
    data: import('./types').RenderRequest,
  ...
  async getRenderStatus(projectId: string): Promise<import('./types').RenderResponse> {
    return this.request(`/api/projects/${projectId}/render/status`);
  }

  getRenderDownloadUrl(projectId: string): string {
    return `${this.getBaseUrl()}/api/projects/${projectId}/render/download`;
  }
}
