...
function getStdioShimCommand(): string {
  if (typeof navigator === 'undefined') {
    return '/Applications/VideoGen Studio.app/Contents/MacOS/videogen-mcp';
  }

  const platform = `${navigator.platform} ${navigator.userAgent}`.toLowerCase();
  if (platform.includes('win')) {
    return 'C:\\Program Files\\VideoGen Studio\\videogen-mcp.exe';
  }
  if (platform.includes('linux')) {
    return '/opt/videogen/videogen-mcp';
  }
  return '/Applications/VideoGen Studio.app/Contents/MacOS/videogen-mcp';
}
 ...
 * existing VideoGen server; this page is the agent-onboarding surface.
 */
      ...
      <div className="flex-1 min-w-0 max-w-2xl space-y-8">
        ...
        >
          ...
          <SnippetRow
            ...
            snippet={`claude mcp add videogen --transport http --url ${mcpUrl} --header "X-Videogen-Client-Id: claude-code"`}
          />
          <SnippetRow
            ...
            snippet={JSON.stringify(
              {
                mcpServers: {
                  videogen: {
                    ...
                    env: { VIDEOGEN_CLIENT_ID: 'claude-code' },
                  },
                },
              },
              null,
      ...
      <aside className="hidden lg:block w-[280px] shrink-0 space-y-6 sticky top-0">
        ...
        <div className="space-y-2">
          ...
          <ul className="text-sm text-muted-foreground space-y-1.5 leading-relaxed">
            <li>
              <code className="text-accent">videogen.speak</code>
              <div>{t('settings.mcp.sidebar.tools.speak')}</div>
            </li>
            <li>
              <code className="text-accent">videogen.transcribe</code>
              <div>{t('settings.mcp.sidebar.tools.transcribe')}</div>
            </li>
            <li>
              <code className="text-accent">videogen.list_captures</code>
              <div>{t('settings.mcp.sidebar.tools.listCaptures')}</div>
            </li>
            <li>
              <code className="text-accent">videogen.list_profiles</code>
