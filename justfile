# VideoGen Studio development commands
...
# Start backend (if not already running) + web app for development
dev: dev-web
...
# Build everything (server binary + web app)
...
build-server: _ensure-venv
    ...
    $dest = "$env:LOCALAPPDATA/com.videogen.studio/backends/cuda"; \
    Copy-Item "backend/dist/videogen-server-cuda/*" $dest -Recurse -Force; \
...
[unix]
db-reset:
    rm -f {{ backend_dir }}/data/videogen.db
    just db-init

[windows]
db-reset:
    if (Test-Path "{{ backend_dir }}/data/videogen.db") { Remove-Item -Force "{{ backend_dir }}/data/videogen.db" }
    just db-init
